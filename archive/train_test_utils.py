...
def train(
    ...
    **kwargs,
    ...
    """
    assert (
        verbose_interval is None
    ) or verbose_interval > 0, (
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if uncertainty:
            pred, uncertainty = calc_prob_uncertinty(
        ...
        correct += np.sum(
            np.array(pred.detach().cpu().numpy())
    ...
    if verbose:
        print(
            "\nTrain set: Average loss: {:.4f} ({:.3f} sec) Accuracy: {:.3f}\n".format(
                loss_avg, endtime - starttime, train_acc
            )
        )
