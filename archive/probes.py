...
class LinearProbeClassification(nn.Module):
    def __init__(
        ...
        if logistic:
            self.proj = nn.Sequential(
                nn.Linear(self.input_dim, self.probe_class), nn.Sigmoid()
            )
        elif Relu:
            self.proj = nn.Sequential(
                nn.Linear(self.input_dim, self.probe_class), nn.ReLU(True)
            )
        elif TanH:
            self.proj = nn.Sequential(
    ...
    def forward(self, act, y=None):
        ...
        logits = self.proj(
            act
        )  # .reshape(-1, self.probe_number, self.probe_class)  # [B, C]
        if y is None:
            return logits, None
        else:
