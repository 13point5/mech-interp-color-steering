...
def remove_last_k_words(s, k):
    ...
    if "[/INST]" in words:
        last_inst_index = max(
            [i for i, word in enumerate(words) if word == "[/INST]"]
        )
    else:
        last_inst_index = -1
...
def split_conversation(
    text, user_identifier="HUMAN:", ai_identifier="ASSISTANT:"
):
    user_messages = []
    assistant_messages = []
    ...
    for line in lines:
        ...
        if line.startswith(user_identifier):
            if current_assistant_message:
                ...
                current_assistant_message = ""
            current_user_message += (
                line.replace(user_identifier, "").strip() + " "
            )
            ...
            if current_user_message:
                ...
                current_user_message = ""
            current_assistant_message += (
                line.replace(ai_identifier, "").strip() + " "
            )
...
class TextDataset(Dataset):
    def __init__(
        ...
        label_idf="_age_",
        label_to_id=None,
        convert_to_llama2_format=False,
        user_identifier="HUMAN:",
        ai_identifier="ASSISTANT:",
        control_probe=False,
        additional_datas=None,
    ...
    def _load_in_data(self):
        for idx in tqdm(range(len(self.file_paths))):
            ...
            int_idx = file_path[
                file_path.find("conversation_") + len("conversation_") :
            ]
            ...
            if self.convert_to_llama2_format:
                if "### Human:" in text:
                    ...
                    user_msgs, ai_msgs = split_conversation(
                        text, self.user_identifier, self.ai_identifier
                    )
                messages_dict = []
                ...
                if (
                    self.if_remove_last_ai_response
                    and messages_dict[-1]["role"] == "assistant"
                ):
            ...
            if (
                self.new_format
                and self.if_remove_last_ai_response
                and self.include_inst
            ):
                ...
                text = text[
                    text.find("<s>") + len("<s>") : text.rfind("[/INST]") - 1
                ]

            label = file_path[
                file_path.rfind(self.label_idf)
                + len(self.label_idf) : file_path.rfind(".txt")
            ]
            ...
            if self.if_augmented:
                if self.residual_stream:
                    for layer_num in range(41):
                        last_acts.append(
                            output["hidden_states"][layer_num][:, -self.k :]
                            .detach()
                            .cpu()
                            .clone()
                            .to(torch.float)
